...
interface EditHomeworkDialogProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (updatedHomework: Homework) => void;
}

export const EditHomeworkDialog: React.FC<EditHomeworkDialogProps> = ({
  homework,
  isOpen,
  onClose,
  onUpdate,
  ...
  const [dueDate, setDueDate] = useState(
    homework?.dueDate ? homework.dueDate.toISOString().split('T')[0] : ''
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError('Errore durante l\'aggiornamento del compito');
    } finally {
      setIsLoading(false);
    }
  };

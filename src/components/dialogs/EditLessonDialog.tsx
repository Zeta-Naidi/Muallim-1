...
export const EditLessonDialog: React.FC<EditLessonDialogProps> = ({
  ...
  const [date, setDate] = useState(
    lesson?.date ? lesson.date.toISOString().split('T')[0] : ''
  );
  ...
  React.useEffect(() => {
    if (lesson) {
      ...
      setDate(lesson.date.toISOString().split('T')[0]);
      setTopics(lesson.topics.length > 0 ? lesson.topics : ['']);
    }
  }, [lesson]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError('Errore durante l\'aggiornamento della lezione');
    } finally {
      setIsLoading(false);
    }
  };
